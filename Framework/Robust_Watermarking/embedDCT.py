...
def main(path, watermark):
    print("Embedding watermark in " + path[9:])
    random.seed(0)

    key_info = {}
    img_host = cv2.imread(path, cv2.IMREAD_COLOR)
    img_mark = cv2.imread(watermark, cv2.IMREAD_GRAYSCALE)
    ...
    #print(path[17:0])
    cv2.imwrite("./DCTWatermarkingResults/Watermarked/"+"DCTMode1_"+path[9:], img_host)
    cv2.imwrite("./imagesToAttack/"+"DCTMode1_"+path[9:], img_host)
    res="./DCTWatermarkingResults/Watermarked/"+"DCTMode1_"+path[9:]
